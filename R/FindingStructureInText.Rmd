...
Diag2 <- ripsDiag(hom, maxdimension, DiagLim, printProgress = FALSE)
...
kmeans <- knnDE(bettiNum_list, 5, nstart = 5 ,iter.max = 15)
#View(text_data)
```
...
```{r}
DistMatrix = matrix(1:2500, nrow = 50, ncol = 50)
for (i in (1:nrow(DistMatrix))){
  for (j in (1:ncol(DistMatrix))){
    DistMatrix[i,j] <- bottleneck()
  }
}
...
text_data$Category_TDA[text_data$Betti_Num == 4 | text_data$Betti_Num == 5 | text_data$Betti_Num == 6] = 'sport'
text_data$Category_TDA[text_data$Betti_Num >= 9] = 'tech'
text_data$Category_TDA[text_data$Betti_Num == 7 | text_data$Betti_Num == 8] = 'politics'
